...
if __name__ == '__main__':
    ...
    # 添加local_rank参数（torch.distributed.launch会自动传入）
    # parser.add_argument('--local_rank', type=int, default=-1, help='Local rank for distributed training')
    parser.add_argument('--local_rank', '--local-rank', type=int, default=-1, help='DDP parameter, read from environment variable LOCAL_RANK')
    ...
    # parser.add_argument('--checkpoints', type=str, default='/root/OpenLTM/checkpoints/', help='location of model checkpoints')
    ...
    parser.add_argument('--output_len', type=int, default=96, help='output length')
    ...
    # # ===== 新增：将相对路径转换为绝对路径（相对于 run.py 所在目录） =====
    # if not os.path.isabs(args.checkpoints):
    #     # 获取 run.py 的绝对路径
    #     run_dir = os.path.dirname(os.path.abspath(__file__))
    #     # 构建绝对路径：run_dir + args.checkpoints
    #     args.checkpoints = os.path.normpath(os.path.join(run_dir, args.checkpoints))
    # # =================================================================
    ...
    if args.dp:
        ...
        ip = os.environ.get("MASTER_ADDR", "10.200.8.31")
