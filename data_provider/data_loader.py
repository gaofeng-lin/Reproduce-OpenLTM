...
class UnivariateDatasetBenchmark(Dataset):
    ...
    def __read_data__(self):
        ...
        if self.data_type == 'ETTh' or self.data_type == 'ETTh1' or self.data_type == 'ETTh2':
            # 按照小时来划分，24表示一天有24小时，乘以30就表示30天，每个数据集的划分是12个月训练，4个月验证，4个月测试
        ...
        # set_type: 0-train, 1-val, 2-test
        # 以ETTh1为例，if set_type=0, border1=0,border2=8640
        ...
        # train dataset, self.data_x的维度[8640,7]
        self.data_x = data[border1:border2]
        self.data_y = data[border1:border2]
        
        # n_var表示变量的个数，比如有多少个传感器
        self.n_var = self.data_x.shape[-1]
        
        # seq_len变为输入长度更好理解，n_timepoint就是可以使用多少个不重叠的样本
    ...
    def __getitem__(self, index):
        ...
        if not self.nonautoregressive:
            ...
            # 这个是更详细的定义，和上面的r_end定义相同
            r_end = s_begin + self.seq_len + self.output_token_len
        ...
        # 输出序列​​紧接​​输入序列，模型学习从​​历史序列​​预测​​紧接着的下一个时间步​
            ...
            seq_x = self.data_x[s_begin:s_end, feat_id:feat_id+1]
            seq_y = self.data_y[r_begin:r_end, feat_id:feat_id+1]
        # seq_x_mark和seq_y_mark是时间特征，这里先用0代替
